...
export function parseGeminiSecret(secretString: string | undefined) {
  if (!secretString) {
  ...
  let secret: unknown;
  try {
    secret = JSON.parse(secretString) as unknown;
  } catch {
  ...
  return secret.GEMINI_API_KEY.trim();
}
...
async function loadGeminiSecret() {
  ...
  process.env.GEMINI_API_KEY = parseGeminiSecret(response.SecretString);
}
