...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="scroll-mt-20 bg-slate-50 py-24 sm:py-32">
      <div className="mx-auto grid max-w-6xl gap-12 px-5 sm:px-6 lg:grid-cols-[.75fr_1.25fr] lg:px-8">
        <div>
          ...
          <h2 className="mt-4 text-4xl font-bold tracking-[-0.03em]">Questions, answered.</h2>
          <p className="mt-5 leading-7 text-slate-600">Everything you need to know before starting your first practice interview.</p>
        </div>
        <div className="space-y-3">
          {faqs.map(([question, answer], index) => {
            const isOpen = openIndex === index;
            return (
              <div key={question} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <button
                  type="button"
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                  className="flex w-full items-center justify-between gap-4 p-5 text-left font-semibold text-slate-900"
