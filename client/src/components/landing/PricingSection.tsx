    ...
    <section id="pricing" className="scroll-mt-20 py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-blue-600">Simple pricing</p>
          <h2 className="mt-4 text-4xl font-bold tracking-[-0.03em] sm:text-5xl">Start free. Grow when you’re ready.</h2>
          <p className="mt-5 text-lg text-slate-600">No payment setup is required to begin practicing.</p>
        </div>
        <div className="mt-14 grid items-stretch gap-6 lg:grid-cols-3">
          {plans.map((plan) => (
            <article
              ...
              className={`relative flex flex-col rounded-3xl border p-7 transition hover:-translate-y-1 ${
                "featured" in plan && plan.featured
                  ? "border-blue-500 bg-slate-950 text-white shadow-2xl shadow-blue-950/20 lg:-translate-y-3 lg:hover:-translate-y-4"
            ...
            >
              ...
              <p className={`mt-3 text-sm ${"featured" in plan && plan.featured ? "text-slate-400" : "text-slate-500"}`}>
                {plan.description}
              </p>
              ...
              <ul className="flex-1 space-y-3">
                {plan.features.map((feature) => (
                  <li key={feature} className={`flex gap-3 text-sm ${"featured" in plan && plan.featured ? "text-slate-300" : "text-slate-600"}`}>
